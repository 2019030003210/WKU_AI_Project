...
def load_patient(base_path, pat_num):
    ...
    # f_ecg = base_path + 'data/TrainingSet/' + 'data/A' +  str(pat_num).zfill(2) + '.mat' #此处换成CycleGan后的ECG
    # x = loadmat(f_ecg)
    # print(x)
    f_ecg = base_path + 'data/TrainingSet/' + 'dataCycleGan/A' +  str(pat_num).zfill(2) + '.mat'
    x = loadmat(f_ecg)
    print(f_ecg)

    # print(x)
    # ecg = np.asarray(x['ecg'], dtype=np.float64)
    ecg = np.asarray(x['gan_outputs'], dtype=np.float64)
...
def test_selfONN(pat_num, q, run = 1, base_path = '../', win_size = 8000, threshold = 0.5):
    ...
    ecg, R_ann, S_ann, V_ann = load_patient(base_path, pat_num) # 将病人原始ECG信号，以及对应病人的标准R，S，V信号，共同读出
